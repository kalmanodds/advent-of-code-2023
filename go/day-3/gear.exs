defmodule Gear do
  def main() do
    {_status, content} = File.read("_inputs/input.txt")
  ...
  def parse_numbers(line, index) when line !== "" do
    case Integer.parse(line) do
      ...
      :error ->
        ...
        remaining_line = String.slice(line, 1, line_length)
  ...
  def parse_symbols(line, index) when line !== "" do
    case Integer.parse(line) do
      {_number, remaining_line} ->
        number_length = String.length(line) - String.length(remaining_line)
        parse_symbols(remaining_line, index + number_length)
      :error ->
        {symbol, remaining_line} = String.split_at(line, 1)
        case symbol do
          "." -> parse_symbols(remaining_line, index + 1)
          _ -> [{index, symbol} | parse_symbols(remaining_line, index + 1)]
        end
    end
  end
